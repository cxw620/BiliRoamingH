        ...
        if (defaultSpeed != 0F) {
            instance.playerCoreServiceV2Class?.hookAfterMethod(
                ...
                if (currentPlaybackSpeed == 1.0f) {
                    instance.setPlaybackSpeed()?.let { mName ->
                        it.args[0].callMethod(mName, defaultSpeed)
                        Log.toast("已设置 $defaultSpeed 倍速")
                    }
                    instance.theseusPlayerSetSpeed()
                        ?.let { mName -> it.args[0].callMethod(mName, defaultSpeed) }
                }
            }
        }
        if (longPressSpeed != 0F) {
            instance.hookInfo.playbackSpeed.tripleSpeedServiceList.forEach {
                it.class_.from(mClassLoader)?.hookBeforeMethod(
                    ...
                    if (speed == 2.0F || speed == 3.0F) param.args[0] = longPressSpeed
                }
            }
        }
            ...
            clazz?.hookAfterAllConstructors { param ->
                ...
                newSpeedTextGroup.children.forEachIndexed { index, view ->
                    when (index) {
                        ...
                        newSpeedTextGroup.childCount - 1 -> view.setPadding(
                            16.dp,
                            4.dp,
                            12.dp,
                            4.dp
