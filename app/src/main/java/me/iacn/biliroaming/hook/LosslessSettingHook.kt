...
import me.iacn.biliroaming.utils.biliPrefs
...
class LosslessSettingHook(classLoader: ClassLoader) : BaseHook(classLoader) {
    ...
    override fun startHook() {
        if (!sPrefs.getBoolean("remember_lossless_setting", false)) return
        instance.playURLMossClass?.run {
            hookBeforeMethod(
                ...
                param.args[1] = param.args[1].mossResponseHandlerProxy {
                    it?.callMethod("getPlayConf")?.callMethod("getLossLessConf")
                        ?.callMethod("getConfValue")?.callMethod("setSwitchVal", losslessEnabled)
                }
            }
            hookBeforeMethod(
                "playConfEdit", "com.bapis.bilibili.app.playurl.v1.PlayConfEditReq"
            ) { param ->
                param.args[0].callMethodAs<List<Any>>("getPlayConfList").firstOrNull {
                    it.callMethodAs<Int>("getConfTypeValue") == 30 // LOSSLESS
                }?.callMethod("getConfValue")?.callMethodAs<Boolean>("getSwitchVal")
            ...
            hookAfterMethod(
                ...
                param.result?.callMethod("getPlayConf")
                    ?.takeIf { it.callMethodAs("hasLossLessConf") }?.callMethod("getLossLessConf")
                    ?.callMethod("getConfValue")?.callMethod("setSwitchVal", losslessEnabled)
            }
        }
        instance.playerMossClass?.run {
            ...
            hookBeforeMethod(
                ...
                param.args[1] = param.args[1].mossResponseHandlerProxy { originalResp ->
                    originalResp?.callMethod("getPlayDeviceConf")
                        ?.callMethodAs<LinkedHashMap<Int, *>>("internalGetMutableDeviceConfs")
                        ?.let {
                            it[30/*LOSSLESS*/]?.callMethod("getConfValue")
                                ?.callMethod("setSwitchVal", losslessEnabled)
                        }
